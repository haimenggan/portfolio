...
export default function ResumeSharePage() {
  ...
  useEffect(() => {
    ...
    const getTargets = () =>
      navItems
        .map((item) => ({ id: item.id, el: document.getElementById(`${anchorPrefix}-${item.id}`) }))
        .filter((t) => t.el);

    const update = () => {
      const targets = getTargets();
      if (!targets.length) return;
      // Find the section whose top is at or just above 35% down the viewport
      const threshold = window.innerHeight * 0.35;
      let best = targets[0];
      for (const t of targets) {
        const top = t.el.getBoundingClientRect().top;
        if (top <= threshold) best = t;
        else break;
      }
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Shared resume generated by haimeng's portfolio" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="preload" href="https://res.cloudinary.com/dvpd0p6si/raw/upload/v1773607605/pe3l1rmtcy65sozxgjgo.glb" as="fetch" crossOrigin="anonymous" />
        <link rel="preload" href="/hero/mr-mime-pokemon-reboot.glb" as="fetch" crossOrigin="anonymous" />
      </Head>
      <div
        ...
        {!ready ? (
          ...
          <>
            <ResumeWebsite
              ...
              projectPageBase={`/resume/${slug}/project`}
            />
          </>
        )}
        </main>
